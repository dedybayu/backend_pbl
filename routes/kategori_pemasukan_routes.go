...
func SetupKategoriPemasukanRoutes(api *gin.RouterGroup, kategoriPemasukanController *controllers.KategoriPemasukanController, authMiddleware *middleware.AuthMiddleware) {
	kategori := api.Group("/kategori-pemasukan")
	{
		...
		kategori.GET("", authMiddleware.RequireLevel(1, 2, 3), kategoriPemasukanController.GetAllKategoriPemasukan)
		kategori.GET("/dropdown", authMiddleware.RequireLevel(1, 2, 3), kategoriPemasukanController.GetKategoriPemasukanDropdown)
		kategori.GET("/:id", authMiddleware.RequireLevel(1, 2, 3), kategoriPemasukanController.GetKategoriPemasukanByID)
